...
import { useXMTP } from "@/contexts/XMTPContext" // 修正 useXMTP 的导入路径
...
export function FriendsList({ onStartChat }: FriendsListProps) {
  ...
  const { client: xmtpClient, loading: loadingXmtpClient } = useXMTP() // Get client and its loading state
  const { conversations, loading: loadingConversations } = useConversations() // Use useConversations
  const [friendXmtpStatus, setFriendXmtpStatus] = useState<Record<string, boolean>>({}) // Store XMTP status
  ...
  // Check if friend is on XMTP network
  useEffect(() => {
    ...
    const checkXmtpStatus = async () => {
      ...
      for (const friendAddr of friends) {
        try {
          const isOnXmtp = await xmtpClient.canMessage(friendAddr)
          statuses[friendAddr] = isOnXmtp
        } catch (e) {
    ...
    // Can set a timer to check periodically, but XMTP canMessage might have rate limits
    // const interval = setInterval(checkXmtpStatus, 60000);
    // return () => clearInterval(interval);
  }, [xmtpClient, friends, loadingXmtpClient]) // Added loadingXmtpClient to dependencies

  if (isLoading || loadingConversations || loadingXmtpClient) {
    // Added loadingXmtpClient
    return (
      <div className="p-4">
    ...
    <div className="p-4 space-y-2">
      ...
      {friends.map((friendAddress) => (
        <FriendItem
          ...
          isOnXmtp={friendXmtpStatus[friendAddress]} // Pass XMTP status
        />
      ))}
    </div>
  )
