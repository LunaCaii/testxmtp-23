...
import type { DecodedMessage, Conversation, ContentTypeId } from "@xmtp/browser-sdk" // Import ContentTypeId
...
interface ChatWindowProps {
  ...
  send: (content: string | MediaContent, contentType?: ContentTypeId) => Promise<void> // From useConversation
...
export function ChatWindow({
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <div className="flex items-center space-x-3">
            <div className="relative">
              <Avatar className={`${isMobile ? "h-8 w-8" : "h-10 w-10"}`}>
                <AvatarImage src={friendProfile?.avatarUrl || "/placeholder.svg"} />
                <AvatarFallback>{friendProfile?.nickname?.slice(0, 2).toUpperCase() || "??"}</AvatarFallback>
              </Avatar>
              {isOnline && <Circle className="absolute -bottom-1 -right-1 h-3 w-3 fill-green-500 text-green-500" />}
            </div>
            <div>
              <h2 className={`font-medium text-gray-900 ${isMobile ? "text-sm" : ""}`}>
      ...
      {/* Message List */}
      <div className={`flex-1 overflow-y-auto space-y-4 bg-gray-50 ${isMobile ? "p-3" : "p-4"}`}>
        {loadingMessages ? (
            ...
            return (
              <div key={message.id} className={`flex ${isOwnMessage ? "justify-end" : "justify-start"}`}>
                <div className={`flex items-end space-x-2 ${isMobile ? "max-w-[85%]" : "max-w-xs lg:max-w-md"}`}>
                  ...
                  {isOwnMessage && (
                    <Avatar className="h-6 w-6">
                      <AvatarImage src={myProfile?.avatarUrl || "/placeholder.svg"} />
                      <AvatarFallback className="text-xs">
      ...
      {showFileUpload && (
        <div className="bg-white border-t border-gray-200 p-4">
          <FileUpload onFileUploaded={handleFileUploaded} disabled={loadingMessages || sending} />
        </div>
      )}
      ...
      <div className="bg-white border-t border-gray-200 p-4">
        ...
        <div className="flex items-center justify-between mt-2">
          <p className={`text-gray-500 ${isMobile ? "text-xs" : "text-xs"}`}>消息通过XMTP实时同步，文件存储在IPFS</p>
